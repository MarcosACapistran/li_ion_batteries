...
if __name__=="__main__":
    ...
    plt.plot(tim,data,'b',label='data')

    
    def init():
        x0 = np.zeros(4)
        eps = 0.75
        x0[0]+=58000.0*(1+eps*np.random.normal())        
        x0[1]+=20000.0*(1+eps*np.random.normal())
        x0[2]+=29000.0*(1+eps*np.random.normal())
        x0[3]+=35000.0*(1+eps*np.random.normal())        
        return x0
    
    solns = np.zeros((20,len(data)))
    ...
    median = np.median(solns,axis=0)
    plt.plot(tim,median,'r',label='median')
    q1 = np.quantile(solns,0.1,axis=0)
    q2 = np.quantile(solns,0.9,axis=0)
    plt.fill_between(tim,q1,q2,color='k', alpha=0.25)
